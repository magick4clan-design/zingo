    ...
    ? parsed.seasons.map((season) => {
        ...
        const episodes = Array.isArray(seasonRecord.episodes)
          ? seasonRecord.episodes.map((episode) => {
              ...
              return {
                ...episodeRecord,
                downloadLinks: parseDownloadLinks(episodeRecord.downloadLinks),
              };
            })
...
router.get('/', async (req, res) => {
  try {
    ...
    if (year) {
      where.releaseYear = parseInt(year as string);
    }
    if (rating) {
      where.imdbRating = { gte: parseFloat(rating as string) };
    }
